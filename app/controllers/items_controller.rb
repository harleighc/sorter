class ItemsController < ApplicationController
  ...
  before_action :save_my_previous_url, only: [:index, :expand]
  ...
  def test
    ...
    @i = (params[:id].to_i)
    @t = (params[:id].to_i) + 30
    @contextbot = Item.where(:identifier => current_user.id ).where(:id => @b...@m)
    @contextmid = Item.where(:identifier => current_user.id ).where(:id => @i )
    @contexttop = Item.where(:identifier => current_user.id ).where(:id => @mm...@t)
 end
 ...
 def edit_gst
  ...
  def update_gst
    ...
    old_items_status = Item.where(:identifier => current_user.id ).where(:category => params[:cat])
  ...
  def update_category
     ...
     old_items_status = Item.where(:identifier => current_user.id ).where(:category => params[:cat].strip).where(:item => params[:item].strip)
     updated_items_status = old_items_status.update_all(:status => 0)
     old_items_mult = Item.where(:identifier => current_user.id ).where(:category => params[:cat].strip).where(:item => params[:item].strip)
     updated_items_mult = old_items_mult.update_all(:multiplier => params[:newmult].try(:strip))
     old_items_cat = Item.where(:identifier => current_user.id ).where(:category => params[:cat].strip).where(:item => params[:item].strip)
     updated_items_cat = old_items_cat.update_all(:category => params[:newcat].try(:strip))
     redirect_to session[:my_previous_url]
  end

    def update_confirms
     old_items = Item.where(:identifier => current_user.id ).where(:category => params[:cat].strip).where(:item => params[:item].strip)
...
#   end
  ...
  def index
    ...
    @runningtotalgst = Item.where(:identifier => current_user.id ).sum('value * gst * multiplier')
    @runningtotal = Item.where(:identifier => current_user.id ).sum(:value)
    ...
    @array_of_live_categories = Categorytable.where(:user_id => [0,current_user.id]).pluck(:name) if current_user
    @total_of_live_categories = Item.where(:identifier => current_user.id ).where(:category => @array_of_live_categories).sum(:value)
    @total_from_db = Item.where(:identifier => current_user.id ).sum(:value) # needs an ID
    @uniqcat = Item.where(:identifier => current_user.id ).pluck(:category).uniq
    ...
    @grouped = Item.where("category = ?", params[:cat]).where(:identifier => current_user.id ).group(:item).pluck('item,sum(value),max(status)')
   ...
   # @expandgroupbottom = Item.where("category = ?", params[:cat]).where("item != ?", params[:item]).where(:identifier => current_user.id ).group(:item).pluck('item,sum(value),max(status)')
   # @expandgroupbottom = @expandgroupbottom.sort_by {|x,y,z|[-z,y,x]}
    #consonle @expandgroupbottom Item.select('item, sum(value) as val').where('category = "BANK FEES"').where('item != "a"').group(:item).sum(:value)
      @it = Item.where(:identifier => current_user.id ).all
  ...
  def update
    @distinct = Item.where(:identifier => current_user.id ).uniq.pluck(:category)
  ...
  def destroy
   ...
   def import
     ...
     #@current = current_user[:id].to_i

     Item.import(params[:file], current_user.id)
